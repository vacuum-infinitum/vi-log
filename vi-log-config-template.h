...
// feature-test-macros with their descriptions.
//
...
// To print them in stdout user needs to compile without NDEBUG macro
// and call `vi_log_config_dump()`.
//
...
// that determined by standard-ish NDEBUG macro:
...
//     Implemented as replacement: `vi_log_exec()` -> `vi_discard()`.
...
//     not throwing warnings of 'unused-*' nature.
//
// If this macro is not defined configuration will use a default value:
//   `VI_LOG_LEVEL_TRC` (aka maximum possible level)
#define VI_LOG_LEVEL_LIMIT VI_LOG_LEVEL_TRC

// Defines presence of an out-of-box agent that outputs to stderr.
...
//          feature list from standard library. So your application
//          must either define them before vi-log.h or pass to a compiler.
...
// Defines presence of an out-of-box agent that uses <syslog.h>.
// To add an agent this macro must be defined with a value of `1`.
// If value is `0` or macro is not defined agent is ignored.
//
...
//          feature list from standard library. So your application
//          must either define it before vi-log.h or pass to compiler.
...
// either to total count of used out-of-box agents or a value of `4`
// if none were selected.
//
// To add custom agent `vi_log_agent_add()` should be used.
#define VI_LOG_AGENTS_EXT_COUNT 0

// Defines presence of an out-of-box thread syncronization mechanism
// implemented as `pthread_mutex_t` from <pthread.h>.
